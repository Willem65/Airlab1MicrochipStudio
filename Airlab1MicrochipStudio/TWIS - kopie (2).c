...
void TWIS_Init(uint8_t* Buffer)
{
	...
	TWIE_MASTER_CTRLA = TWI_SLAVE_INTLVL_LO_gc | TWI_SLAVE_DIEN_bm | TWI_SLAVE_APIEN_bm | TWI_SLAVE_PIEN_bm | TWI_SLAVE_ENABLE_bm;

	// Set the slave address
	TWIE_SLAVE_ADDR = (TWI_SLAVE_ADDRESS << 0x01);

	// Use no mask or second device address
	TWIE_SLAVE_ADDRMASK = 0x00;
}
